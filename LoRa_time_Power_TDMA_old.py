...
# -------   execution time   ---------------------------------------------------------
# according to our definition, exe time is equal to required amount of time to send 50 byte of data by node and receive 16 byte of ack from gateway
# plus safty guard percent of exe time 

send_rec_time = time_on_air + ack_time_on_air
exe_time = send_rec_time + (send_rec_time * safety_guard)

sync_tx_rx_time = sync_ready_time_on_air + sync_rec_time_on_air
...
def sending_data_energy_consumption_curr(TP = 17):
...
def sending_ready_energy_consumption_curr(TP = 17):
    energy_usage_curr = sync_ready_time_on_air * lora_transmit_current(TP)  # energy unit is milli amper
    return energy_usage_curr  # energy unit is milli joule


def receiving_energy_consumption_curr(BW = 125000):
    return lora_receive_current(BW) * ack_time_on_air


def receiving_ready_energy_consumption_curr(BW = 125000):
